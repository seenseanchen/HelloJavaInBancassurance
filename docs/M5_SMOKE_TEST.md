...
### 4.3 步驟二：執行衝突示範腳本（可重複執行）

> ⚠️ **不要寫死 expectedVersion**。版本號每次成功變更後都會 +1，寫死就會踩到 412。
> 下方腳本每次都動態從 API 讀出當前版本，再同時發出兩支 PATCH。

```bash
# ── 每次執行前動態取得當前 version ──────────────────────────────────
VER=$(curl -s "$host/api/policies/$id" | grep -o '"version":[0-9]*' | grep -o '[0-9]*')
echo "Current version = $VER"

# ── ★ 地址必須帶時間戳，確保每次都是全新值 ──────────────────────────
# 原因：Hibernate dirty check 比對「載入時的快照」vs「現在的值」。
#   若你用同一個字串 (例如 "A 搶先改的地址") 反覆送，而 DB 已經是那個值，
#   则 setBillingAddress 前後一樣 → 沒有 dirty → 不下 UPDATE → 不觸發 @Version！
#   這不是 bug，是 Hibernate dirty checking 正確運作的結果。
#   加上時間戳讓每次地址都不同，確保必定 dirty。
TS=$(date +%H%M%S)
DATA_A="{\"expectedVersion\": $VER, \"newAddress\": \"A改-${TS}\", \"reason\": \"demo A\"}"
DATA_B="{\"expectedVersion\": $VER, \"newAddress\": \"B改-${TS}\", \"reason\": \"demo B\"}"
URL="$host/api/policies/$id/address"

echo "DATA_A = $DATA_A"
echo "DATA_B = $DATA_B"

# ── A 先送，等 1 秒後才送 B ──────────────────────────────────────────
#   T=0s   A 抵達 server → loadEntity → sleep 3s 開始
#   T=1s   B 抵達 server → loadEntity（A 還在睡，READ_COMMITTED 看到的還是 v=VER）→ sleep 3s
#   T=3s   A 醒 → saveAndFlush → COMMIT → version=VER+1
#   T=4s   B 醒 → saveAndFlush → WHERE version=VER → 0 rows → OptimisticLockException → 409 ✓
echo "=== 啟動 A ==="
curl -i -s -X PATCH "$URL" -H 'Content-Type: application/json' -d "$DATA_A" > /tmp/resp_A.txt &

echo "=== 等 1 秒 (確認 A 已進入 server 端 sleep，B 才出發) ==="
sleep 1

echo "=== 啟動 B (在 A 的 sleep 視窗內)==="
curl -i -s -X PATCH "$URL" -H 'Content-Type: application/json' -d "$DATA_B" > /tmp/resp_B.txt &

wait
echo ""
echo "=== Response A ===" && grep -E 'HTTP/|"code"|"version"' /tmp/resp_A.txt
echo ""
echo "=== Response B ===" && grep -E 'HTTP/|"code"|"version"' /tmp/resp_B.txt
```
...
- [x] Flyway V5 跑成功 (`flyway_schema_history` 有第 5 筆)
- [x] `policy_change_log` 與 `idempotency_record` 兩表存在
- [x] GET /policies/{id} 回應含 `ETag: "0"` header
- [x] PATCH /address 成功 → version 變 1，change log 多 1 筆 ADDRESS
- [x] PATCH /beneficiaries 全量替換成功，舊受益人 (王太太) 從 `policy_beneficiary` 消失（軟刪可選；本系統真 DELETE 因為 orphanRemoval 沒看 is_deleted）
- [x] If-Match 帶錯 → 412 PRECONDITION_FAILED
- [x] 對 LAPSED 保單變更 → 409 INVALID_POLICY_STATE
- [x] 受益人比例 ≠ 100 → 422 BUSINESS_RULE_VIOLATION
...
- [x] GET /policies/{id}/changes 回 3+ 筆，按時間倒序
