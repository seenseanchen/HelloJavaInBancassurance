...
async function fetchPolicy() {
  ...
  if (authStore.authMode === 'mock') {
    policy.value = null
    errorMessage.value =
      '目前是 mock 模式，請改用後端 CSR / ADMIN 帳號登入後再操作地址變更。'
    return
  }
  ...
  try {
    ...
    form.newAddress = response.policy.billingAddress
  } catch (error) {
...
async function submitChange() {
  ...
  const newAddress = form.newAddress.trim()
  if (newAddress.length === 0) {
    errorMessage.value = '新地址不可為空。'
    return
  }
  ...
  try {
    const response = await changePolicyAddress(
      policyId.value,
      {
    ...
    const message = apiError?.message ?? '地址變更失敗，請稍後再試。'
...
watch(
  () => route.params.id,
  async () => {
    if (route.name !== 'policy-change-address') {
      return
    }
    await fetchPolicy()
  },
  ...
  <main class="min-h-screen px-4 py-8">
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <el-alert
        ...
        title="目前角色沒有地址變更權限（需要 CSR / ADMIN）。"
        type="warning"
        show-icon
        :closable="false"
      />
        ...
        <BaseCard>
          <div class="grid gap-3 tablet:grid-cols-2">
            ...
            <div>
              ...
              <p class="text-body text-neutral-900">{{ policy.holderName }}</p>
            </div>
            <div>
